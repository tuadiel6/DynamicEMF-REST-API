...
String modelDirectory = "/Users/adielt./Desktop/WebServiceConference/Dynamic-EMF-REST-API/src/main/resources/model";
	...
	public List<EObject> loadXMI(String packageName, String className) 
	    ...
	    for (String modelPath : modelPaths) {
	        ...
	        } else if (!validationMessage.isEmpty() || validationMessage.contains("Severity: 4")) {
	        	 throw new ValidationException("Something went wrong in the model:" + validationMessage);
	        }
	            //===end validation              
	...
	*/
	...
	public static String validateObject(Resource resource) {
	    ...
	    if (diagnostic.getSeverity() == Diagnostic.ERROR || diagnostic.getSeverity() == Diagnostic.WARNING) {
	        //validationMessage.append("VALIDATION MESSAGE: The model is not valid. Details:\n");
	        for (Iterator i = diagnostic.getChildren().iterator(); i.hasNext();) {
	            Diagnostic childDiagnostic = (Diagnostic) i.next();
	            switch (childDiagnostic.getSeverity()) {
	                case Diagnostic.ERROR:
